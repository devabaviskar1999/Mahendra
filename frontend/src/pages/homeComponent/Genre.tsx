...
const Genre = ({ imgUrl, title, description }: FirstPropTypes) => {
  return (
    <Link to={navigate(title)}>
      <div className="flex relative flex-col items-center justify-center gap-4 p-4 bg-white rounded-xl shadow-md transition-shadow duration-300 delay-100 ease-in-out hover:shadow-xl hover:cursor-pointer font-sans">
        ...
        <p className="text-sm font-medium sm:text-base md:text-lg text-gray-700 text-left leading-relaxed">
          {description} <span className="text-red-500 text-[10px] sm:text-sm md:text-lg">Tap to view</span>
        </p>
      </div>
    </Link>
  );
